...
- [ ] 1.2 Add `bevy` and `bevy_rapier3d` to `Cargo.toml`.
- [ ] 1.3 Create the basic Bevy `App::new()` with `DefaultPlugins` and `RapierPhysicsPlugin`.
...
- [ ] 3.3 Create a `rover_movement` system that reads `ButtonInput<KeyCode>` (WASD) and applies forces to the rover's `ExternalForce` component.
