...
**Created**: 2026-03-29
**Status**: Partial — sourcing/caching of Modelica source libraries built; no decimation/KTX/GIS.
...
- When resolving dependencies, the explicit provisioning pipeline checks the shared OS cache returned by `lunco_assets_core::cache_dir()` (for example, `~/.cache/lunco/` on Linux). Missing data is downloaded only after an explicit CLI, UI, or Rhai request; scene loading never phones home.
