...
**Input**: Expose all openable model sources (Twin folder, bundled examples, admitted Modelica source libraries, ephemeral docs) and current workspace state through the API, with a single scheme-aware `Open` command.
...
### User Story 4 - List Modelica Source-Library Classes With Filters and Pagination (Priority: P2)

As an AI agent
I want to enumerate admitted Modelica source-library classes with prefix, category, and "examples-only" filters, paginated
So that I can find a class to open without reading 2500 entries at once

**Why this priority**: source libraries are large enough that a single dump would blow agent context. Pagination + filters are required, not optional.

**Independent Test**: Issue `list_library(filter:{examples_only:true, prefix:"Modelica.Blocks"}, limit:50)`, receive ≤50 example classes from `Modelica.Blocks.*` plus a `next_cursor` for the rest.

**Acceptance Scenarios**:

1. **Given** a source library is available, **When** I call `list_library(limit:200)`, **Then** I receive 200 entries plus a `next_cursor` token I can pass back for the next page.
2. **Given** I pass `filter:{prefix:"Modelica.Electrical"}`, **When** I call `list_library`, **Then** every entry returned has a qualified name starting with `Modelica.Electrical`.
3. **Given** I pass `filter:{examples_only:true}`, **When** I call `list_library`, **Then** every entry returned has `.Examples.` in its qualified path.
4. **Given** I pass `filter:{category:"Blocks"}`, **When** I call `list_library`, **Then** every entry has `Blocks` as its top-level package segment after the source root.
5. **Given** a source library has not finished loading, **When** I call `list_library`, **Then** the call still returns valid authoritative data, even at the cost of blocking.
...
So that I do not need to know whether the target is bundled, from an admitted source library, on disk, or already open

**Why this priority**: Collapses the three existing open commands (`OpenFile`, `OpenClass`, `OpenExample`) plus the missing `bundled://` and `mem://` paths into one entry point.

**Independent Test**: Issue `open("bundled://RC_Circuit.mo")`, `open("Modelica.Blocks.Examples.PID_Controller")`, `open("/abs/path/foo.mo")`, `open("mem://Untitled3")`. Verify each lands the correct document on screen.
...
2. **Given** I pass a qualified Modelica name with no scheme (dot-separated, no `/`), **When** I call `open`, **Then** the call is dispatched to the source-library opener and the class opens as a duplicated Untitled tab.
...
- **FR-004**: `list_library` MUST return a paginated slice of classes from admitted Modelica source libraries with an opaque `next_cursor` token, accepting at minimum `prefix`, `category`, `examples_only` filters and a `limit` parameter (default 200, max 1000). The first call may block briefly on source-library initialization; subsequent calls hit the cached index. A separate readiness endpoint is intentionally not provided because a status check would add a race between status and use.
- **FR-005**: `open(uri)` MUST dispatch on URI scheme: `bundled://` → embedded source as Untitled, qualified name → admitted source-library class, absolute path → file open, `mem://` → focus existing tab.
...
- **`ApiQueryProvider` (new trait, `lunco-api`)**: A registered query provider that answers a typed request for a list of items. Domain capability crates (`lunco-modelica-api`, `lunco-workspace-api`) register implementations for bundled, twin, source-library, and open-document queries. Keeps both `lunco-api` and the Modelica compiler core free of domain-query registration.
- **`ListBundledRequest` / `ListTwinRequest` / `ListLibraryRequest` / `ListOpenDocumentsRequest` / `OpenRequest` (new `ApiRequest` variants)**: Transport-agnostic enum variants. The transport layer parses them from the same `command` field used today; the executor routes them to the registered query providers.
...
- **SC-001**: An agent can answer "what can I open right now?" with a single round trip per source (4 calls maximum: `list_bundled`, `list_twin`, `list_library`, `list_open_documents`).
- **SC-002**: An agent can open any of the four source kinds with a single `open(uri)` call, regardless of source.
- **SC-003**: `list_library` with `limit:200` returns in <50 ms after the source-library index is available.
...
- Full-text search across source libraries or bundled sources (filter-only; no text indexing in this spec).
...
- Source-library loading is asynchronous where possible and the listing provider remains authoritative when it must wait for the index.
...
- Cursor-based pagination is acceptable for source libraries despite being stateless (the cursor encodes the offset and a filter hash).
...
- **P4** — `list_library` provider over the admitted source-library index.
