...
## Problem Statement
Basic multiplayer (`003`) provides single-operator space system control. For professional concurrent engineering — where multiple specialists work on different subsystems of the same space system simultaneously — we need fine-grained **authority delegation** and **role-based access control (RBAC)**.
...
As a space system owner, I want to grant another engineer control over a specific subsystem (e.g., robotic arm) while I retain control of the drive system.

**Acceptance Criteria:**
- Each FSW module (Drive, Arm, Power, Comms, etc.) is independently authorizable.
- The space system owner can grant/revoke subsystem access from the multiplayer UI.
- Multiple engineers can operate different subsystems of the same space system concurrently.
...
- Owners have master authority over their space systems and can delegate subsystem access.
