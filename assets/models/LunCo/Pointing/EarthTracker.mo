...
equation
  ...
  // Tangent-plane separation between boresight and target. The sin(el) factor is
  // the convergence of azimuth lines toward the +Y zenith — a yaw error means
  // less on the sky the higher the dish points. Stays differentiable at zero,
  // which an acos() formulation does not.
  point_error = sqrt(atan2(sin(azimuth.cmd - az), cos(azimuth.cmd - az))^2
                     + (sin(el) * (elevation.cmd - el))^2);
