...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
    ...
    // Normal: perturb the procedural normal toward the map's tangent normal.
    if (mat.weight_normal > 0.0) {
        let tn = textureSample(normal_tex, normal_smp, uv).xyz * 2.0 - 1.0;
        // Map the tangent-space normal into world space via the geometric basis.
        var up = vec3(0.0, 1.0, 0.0);
        if (abs(n.y) > 0.99) { up = vec3(1.0, 0.0, 0.0); }
        let t = normalize(cross(up, n));
        let b = cross(n, t);
    ...
    color = vec4(color.rgb + albedo * SHADOW_FILL * shadow_fill_weight(in.uv), color.a);
#endif
