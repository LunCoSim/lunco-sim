...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
    ...
    if (mat.shadow_cache_on > 0.5) {
        ...
        if (blend > 0.0) {
            ...
            // Floor at 0.15: the multiply is post-lit, so 0 would crush the
            // emissive fill too and shadowed valleys read as void. Real lunar
            // shadow floors near-black, but a readable scene wins here.
    ...
    // Slope hazard from the GEOMETRIC normal (the bump-perturbed `n` would speckle
    // the overlay into micro-noise): green ≤ safe angle → amber → red ≥ cliff angle,
    // via the SAME smoothstep + green/amber/red ramp as
    // `lunco_terrain_core::transfer::{hazard_from_slope, hazard_color}` — so the
    // on-screen colour matches a legend swatch / headless export exactly. Params are
    // uniforms → dropping the cliff angle re-reds steeper ground with no re-bake.
    if (mat.overlay_mode > 0.5 && mat.overlay_opacity > 0.0) {
        let slope = acos(clamp(n_geo.y, -1.0, 1.0));   // 0 = flat, π/2 = vertical
        ...
        if (hi - lo >= 1e-6) {
            ...
            t = t * t * (3.0 - 2.0 * t);               // smoothstep (== hazard_from_slope)
        }
