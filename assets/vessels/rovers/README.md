...
**One parameter set, two wheel kinds.** Raycast and physical (joint) wheels read the SAME attributes through one strict reader (`lunco-usd-sim-authoring`); only force generation differs. Every drivetrain/tire attribute is **required** — a wheel missing any refuses to spawn and the error names all of them. The defaults live in `components/mobility/wheel.usda` (+ tires/suspensions), which every vehicle wheel composes; the vehicle wheel instance also applies the standard wheel/attachment APIs and authors its `physxVehicleWheelAttachment:index`, explicit drive/steer connections, and variants. Composed completeness is pinned by the production `battery_mounts`, `drivetrain_parity`, and `wheel_attachment_contract` Rhai scenarios.

Raycast wheels are query-only: they use an authored support footprint and do not
enter Avian's collision world. Physical wheels must author `PhysicsRigidBodyAPI` and
`PhysicsCollisionAPI` on the wheel geometry; the runtime projects that USD shape and
does not synthesize a collider from the wheel radius or width. Missing or disabled
physical collision authoring is an asset error, not a fallback to a generated shape.
