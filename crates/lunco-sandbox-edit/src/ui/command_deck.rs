    ...
    if let Some(v) = sel {
        if let Ok(spec) = q_spec.get(v) {
            match &spec.0 {
                ...
                other => {
                    view.behaviour_kind = format!("{:?}", other).split('(').next().unwrap_or("?").to_lowercase();
                }
            }
        }
    }
        ...
        ui.horizontal(|ui| {
            ...
            if view.autopilot_engaged {
                ...
                if ui.button("Engage autopilot").clicked() {
                    ctx.defer(move |world| {
                        world.trigger(lunco_autopilot::EngageAutopilot { vessel: v, index: 0, throttle: 0.5, spec_json: String::new() });
                    });
                }
            }
        });
