...
use lunco_terrain_core::{
    HeightSource, normal_at_bounded, square_boundary_height_at, square_boundary_posting_spacing,
    ...
    fn dem_perimeter_uses_every_posting_and_has_closed_corner_topology() {
        ...
        for edge in [DemEdge::Top, DemEdge::Bottom, DemEdge::Left, DemEdge::Right] {
            ...
            assert_eq!(
                edge_vertices.len(),
                res_dem,
                "missing boundary posting on {edge:?}"
            );
            ...
            for &(along, vertex) in &edge_vertices {
                ...
                let expected = square_boundary_height_at(&boundary, x, z, half as f64, res_dem)
                    .expect("boundary source sample");
                assert!((mesh.positions[vertex as usize][1] as f64 - expected).abs() < 1.0e-4);
                assert_eq!(
                    mesh.positions[vertex as usize],
                    mesh.morph_targets[vertex as usize]
                );
                assert_eq!(
                    mesh.normals[vertex as usize],
                    mesh.morph_normals[vertex as usize]
                );
                let posting_spacing =
                    square_boundary_posting_spacing(half as f64, res_dem).unwrap();
                let expected_normal =
                    normal_at_bounded(&boundary, x, z, posting_spacing, half as f64)
            ...
            for pair in edge_vertices.windows(2) {
                ...
                assert_eq!(
                    edge_counts.get(&key),
                    Some(&1),
                    "open or multiply drawn segment on {edge:?}"
                );
            }
        }
        ...
        for triangle in mesh.indices.chunks_exact(3) {
            ...
            let [a, b, c] = points;
            covered_area += 0.5
                * (((b[0] - a[0]) * (c[2] - a[2]) - (b[2] - a[2]) * (c[0] - a[0])).abs() as f64);
            for (a, b) in [
                (triangle[0], triangle[1]),
                (triangle[1], triangle[2]),
                (triangle[2], triangle[0]),
        ...
        for ((a, b), count) in edge_counts {
            ...
            let is_boundary = ((ax + half).abs() < tolerance && (bx + half).abs() < tolerance)
