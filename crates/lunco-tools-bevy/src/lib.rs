...
/// Side-table of the tools that are **executable** (not just discoverable),
/// keyed by [`Tool::name`] — the dispatch index for [`on_tool_fired`].
///
/// It exists because `Arc<dyn Tool>` cannot be cast to `Arc<dyn ExecutableTool>`:
/// `Any` only ever gives back the *concrete* type, so recovering the supertrait
/// from the bevy-free registry would mean enumerating every concrete instrument
/// type here — and any type missed would be silently skipped as "script-only".
/// Recording the trait object at REGISTRATION time instead keeps dispatch open to
/// any `Tool + ExecutableTool` impl, from any crate.
fn executables() -> &'static RwLock<HashMap<String, Arc<dyn ExecutableTool>>> {
    static R: OnceLock<RwLock<HashMap<String, Arc<dyn ExecutableTool>>>> = OnceLock::new();
...
fn on_tool_fired(trigger: On<ToolFired>, world: DeferredWorld) {
    ...
    // Look the tool up in the executable side-table (populated at registration).
    let executable = executables().read().unwrap().get(&ev.tool).cloned();
    let Some(executable) = executable else {
        if lunco_tools::get(&ev.tool).is_some() {
            debug!(
                "[lunco-tools-bevy] tool '{}' is registered but not executable \
                 (script-only); skipped",
                ev.tool
            );
        } else {
...
pub fn register_closure_tool(
    ...
    register_executable(Arc::new(ClosureTool::new(name, functions, exec)));
}
...
mod tests {
    ...
    #[test]
    fn a_bespoke_executable_tool_dispatches_too() {
        // Dispatch must be open to ANY `Tool + ExecutableTool`, not just
        // `ClosureTool`. The old `Any`-downcast bridge enumerated concrete types,
        // so a bespoke instrument like this one was silently skipped as
        // "script-only" — a dropped tool call with no warning.
        ...
        register_executable(Arc::new(Bespoke));
        let mut app = App::new();
