...
use lunco_usd_sim_domain::synthesis::network_facts;
...
fn an_explicitly_resolved_source_class_is_exposed_to_policy_facts() {
    ...
    let network = read_network(&view, &root, &classes)
        ...
        .expect("still a network");
    let facts =
        network_facts(&network, "Rig_System", Some(&classes)).expect("network facts are valid");
        ...
        errors.iter().any(|error| {
            error.path == "/Rig/Battery.info:sourceAsset"
                && error.message.contains(
                    "implementationSource selects sourceAsset but info:sourceAsset is empty",
                )
        }),
