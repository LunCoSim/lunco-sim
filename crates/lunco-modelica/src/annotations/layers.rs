...
impl Icon {
    /// Bounding box over the icon's graphics, in icon coordinates.
    /// `include_text` controls whether `Text` items contribute — they're
    /// excluded for icon-content sizing (`graphics_bbox`) but included for
    /// the full extent (`full_bbox`).
    fn bbox(&self, include_text: bool) -> Option<Extent> {
        let mut found = false;
    ...
    /// Bounding box over non-text graphics (Rectangle/Ellipse/Polygon/Line/Bitmap).
    pub fn graphics_bbox(&self) -> Option<Extent> {
        self.bbox(false)
    }

    /// Bounding box over all graphics, including `Text` extents.
