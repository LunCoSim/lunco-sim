...
pub(crate) fn poll_and_swap_projection(
    ...
    if let Some((gen, task_doc, target, source_hash, scene)) = done_task {
        ...
        let old_origin_to_id: std::collections::HashMap<String, lunco_canvas::NodeId> = docstate
            .canvas
            .scene
            .nodes()
            .filter_map(|(id, n)| n.origin.clone().map(|o| (o, *id)))
            .collect();
        let new_origin_to_id: std::collections::HashMap<String, lunco_canvas::NodeId> = scene
            .nodes()
            .filter_map(|(id, n)| n.origin.clone().map(|o| (o, *id)))
            .collect();
        ...
        docstate
            .canvas
            .tool
            .remap_node_ids(&|old: lunco_canvas::NodeId| id_remap.get(&old).copied());

        let mut scene = scene;
        let (bg_graphics, bg_plot_nodes): (Vec<_>, Vec<_>) = docstate
            .background_diagram
            .read()
            .ok()
            .and_then(|g| {
                g.as_ref()
                    .map(|(_, gfx, plots)| (gfx.clone(), plots.clone()))
            })
            .unwrap_or_default();
        ...
        let source_backed_origins = decorations::emit_diagram_decorations(
            &mut scene,
            &bg_graphics,
            &bg_plot_nodes,
            Some(task_doc),
        ...
        let scene_only_nodes: Vec<lunco_canvas::scene::Node> = docstate
            ...
            .filter(|(_, n)| {
                n.kind == lunco_viz::kinds::canvas_plot_node::PLOT_NODE_KIND
                    && n.origin
                        .as_deref()
                        .map(|o| !source_backed_origins.contains(o))
                        .unwrap_or(true)
            })
            .map(|(_, n)| n.clone())
            .collect();
...
pub(crate) fn trigger_projection_if_needed(
    ...
    let needs_project = {
        ...
        let first_render = !match render_tab_id {
...
fn spawn_projection_task(
    ...
    let resolved = {
        ...
        registry
            ...
            .and_then(|host| {
                host.document()
                    .strict_ast()
                    .map(|ast| (host.document().source_arc(), ast))
            })
    };
    ...
    let (max_nodes, max_duration) = ctx
        .resource::<DiagramProjectionLimits>()
    ...
    let target_class = render_target_ctx(ctx)
        ...
        .or_else(|| crate::sim_default::default_simulation_class_ctx(ctx, doc_id));
    let layout = ctx
        .resource::<crate::ui::panels::canvas_projection::DiagramAutoLayoutSettings>()
        .cloned()
        .unwrap_or_default();

    {
        ...
        let diag =
            decorations::diagram_annotation_for_target(ast_arc.as_ref(), target_class.as_deref());
