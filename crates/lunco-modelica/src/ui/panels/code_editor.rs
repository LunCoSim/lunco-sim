        ...
        let present = ctx.resource_scope::<EditorBufferState, _>(|ctx, buf| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    ...
                    .on_hover_text(
                        "Ctrl/Cmd+V (no permission prompt) or this button (prompts once)",
                    )
                    .clicked()
                {
                    tb_paste = true;
            ...
            let selected_text: Option<String> =
                selection_chars.map(|(s, e)| text.chars().skip(s).take(e - s).collect::<String>());
            ...
            if let Some((cut_start, cut_end)) = cut_range {
                ...
                let still_present = selected_text
                    .as_deref()
                    .map(|sel| {
                        let extracted: String = text
                            .chars()
                            .skip(cut_start)
                            .take(cut_end - cut_start)
                            .collect();
                        extracted == sel
                    })
                    .unwrap_or(false);
            ...
            if !is_ro {
                if let Some(raw_paste) = crate::ui::wasm_clipboard::take_pending_paste() {
                    ...
                    let paste_text = raw_paste.replace("\r\n", "\n").replace('\r', "\n");
                    ...
                    let mut out = String::with_capacity(text.len() + paste_text.len());
...
pub fn commit_pending_buffer(world: &mut World, doc: lunco_doc::DocumentId) -> bool {
    ...
    if let Some(prior) = prior {
        if let Some((range, replacement)) = crate::text_diff::diff_to_edit(&prior, &committed) {
            bevy::log::debug!(
            ...
            match &result {
                ...
                Err(reject) => bevy::log::warn!(
                    "[editor-flush] doc={} apply rejected: {reject:?}",
                    doc.raw()
                ),
            }
            if result.is_ok() {
...
const CONTROL_KEYWORDS: &[&str] = &[
    "if", "then", "else", "elseif", "for", "in", "loop", "while", "when", "elsewhen", "break",
    "return",
];
const OPERATOR_KEYWORDS: &[&str] = &[
    "and", "or", "not", "der", "connect", "time", "true", "false",
];
const BUILTIN_TYPES: &[&str] = &["Real", "Integer", "Boolean", "String", "enum"];
