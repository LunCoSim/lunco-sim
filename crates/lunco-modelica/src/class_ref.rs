...
//!
//! `ClassRef` is the canonical typed identity for classes. Tree clicks,
//! palette drops, drill-ins, typed API commands, and session restore all
//! converge on this one type, so
//! lookups for "open this", "is this loaded?", "what's its
//! documentation?", "where's its source file?" share a single path
//! by construction.
//!
//! ## Serialized scheme → `ClassRef`
//!
//! | Serialized string | `ClassRef` form |
//! |---|---|
...
impl ClassRef {
    ...
    /// Parse a serialized tree-row ID into a `ClassRef`. Returns
