...
fn main() {
    ...
    panic!(
        "lunica_worker is wasm32-only — built into a Web Worker bundle by scripts/build_web.sh."
    );
}
...
mod wasm {
    ...
    fn command_label(cmd: &ModelicaCommand) -> String {
        match cmd {
            ModelicaCommand::Step {
                model_name, entity, ..
            } => format!("Step {model_name} entity={entity:?}"),
            ModelicaCommand::Compile {
                model_name, entity, ..
            } => format!("Compile {model_name} entity={entity:?}"),
            ModelicaCommand::UpdateParameters {
                model_name, entity, ..
            } => format!("UpdateParameters {model_name} entity={entity:?}"),
    ...
    fn run_fast_in_worker(
        ...
        post_log(
            scope,
            format!("run_fast: start run={run_id:?} model={model_name}"),
        );
        ...
        let onmessage = Closure::wrap(Box::new(move |event: MessageEvent| {
            ...
            match envelope {
                WireMessage::Command(cmd) => {
                    ...
                    // Capture the command's lifecycle identity before moving
                    // it into the dispatch closure. The shared panic result
                    // preserves Compile/Step/Reset/source-root semantics.
                    ...
                    match outcome {
                        ...
                        Err(e) => {
                            ...
                            // reproduces this).
                            post_result(&scope, panic_result);
                    ...
                    match lunco_modelica::msl_remote::decompress_parsed_bundle(&bytes) {
                        Ok(decoded) => {
                            match lunco_modelica::msl_remote::deserialize_parsed_bundle(&decoded) {
                                Ok(parsed) => {
                                    let count = parsed.len();
                                    lunco_modelica::msl_remote::install_global_parsed_msl_pub(
                                        parsed,
                                    );
                ...
                WireMessage::CancelRun { run_id } => {
                    ...
                    post_log(
                        &scope_for_cb,
                        format!("cancel requested for run {run_id:?}"),
                    );
                }
