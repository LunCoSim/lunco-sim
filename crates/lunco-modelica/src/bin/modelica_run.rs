            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    "--output" => {
                        let v = iter
                            .next()
                            .unwrap_or_else(|| die("--output requires a path"));
                        output = Some(PathBuf::from(v));
                    }
                    "--input" => {
                        let kv = iter
                            .next()
                            .unwrap_or_else(|| die("--input requires NAME=VALUE"));
        ...
        let mut csv_writer: Option<std::io::BufWriter<std::fs::File>> = match &opts.output {
            Some(path) => {
                let f = std::fs::File::create(path)
                    .unwrap_or_else(|e| die(&format!("failed to create {}: {e}", path.display())));
