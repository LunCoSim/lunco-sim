...
//! [`NetExcluded`], [`PredictedDynamic`], [`NotPredictable`],
//! [`ArticulatedVehicle`], [`ArticulatedLink`], [`OwnedLocally`],
//! [`SkipContentStamp`], [`NetSpawn`], [`PendingReplicatedSpawns`]) are stamped
//! or read by `lunco-usd-sim` / `lunco-scene-commands` / `lunco-cosim` /
//! `lunco-mobility` and friends. Types whose only producer AND consumer is the
