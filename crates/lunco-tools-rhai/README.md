...
- **`refresh(&mut Engine)`** — binds every currently registered tool into the
  ...
  runtime builds a fresh engine and calls `refresh` on registry-generation
  changes because static modules cannot be removed from an existing Rhai engine.
