...
fn test_surface_rot_zero_roll_y_up() {
    ...
    for heading_deg in [0, 45, 90, 135, 180, 270, 315] {
        ...
        for pitch_deg in [-60, -30, 0, 30, 60] {
            ...
            assert!(
                has_no_roll(rot, up_v),
                "Roll violation at h={}° p={}° up=Y",
                heading_deg,
                pitch_deg
            );
        }
...
fn test_surface_rot_idempotent() {
    ...
    for heading in [0.0, 0.5, 1.0, 2.0] {
        for pitch in [-0.3, 0.0, 0.2] {
            ...
            assert!(
                diff < 1e-6,
                "Rotation not idempotent at h={}, p={}: diff={:.8}",
                heading,
                pitch,
                diff
            );
    ...
    for lat in [-80, -45, 0, 45, 80] {
        for lon in [-180, -90, 0, 90, 180] {
            ...
            assert!(
                ...
                "Normal at lat={}, lon={} should be unit length, got {}",
                lat,
                lon,
                n.length()
            );
...
fn test_altitude_surface_50m() {
    ...
    assert!(
        (alt - 50.0).abs() < 1.0,
        "Surface altitude should be 50m, got {}",
        alt
    );
}
...
fn test_altitude_low_orbit_100km() {
    ...
    let alt = (camera_pos - body_pos).length() - MOON_RADIUS;
    assert!(
        (alt - 100_000.0).abs() < 100.0,
        "Orbit altitude should be ~100km, got {}",
        alt
    );
}
...
fn test_altitude_earth_moon_distance() {
    ...
    assert!(
        alt > 383_000_000.0,
        "EM distance should be ~385M m, got {}",
        alt
    );
}
...
fn test_terrain_spawns_on_surface() {
    let alt = 50.0;
    assert!(
        alt < TERRAIN_THRESHOLD,
        "Terrain should spawn at 50m altitude"
    );
}
...
fn test_terrain_spawns_low_orbit() {
    ...
    assert!(
        alt < TERRAIN_THRESHOLD,
        "Terrain should spawn at 50km altitude"
    );
}
...
fn test_terrain_no_spawn_high_orbit() {
    ...
    assert!(
        alt >= TERRAIN_THRESHOLD,
        "Terrain should NOT spawn at 200km altitude"
    );
}
...
fn test_terrain_no_spawn_em_distance() {
    ...
    assert!(
        alt >= TERRAIN_THRESHOLD,
        "Terrain should NOT spawn at Earth-Moon distance"
    );
}
...
fn test_moon_surface_gravity() {
    ...
    assert!(
        (g - 1.625).abs() < 0.05,
        "Moon surface gravity should be ~1.625 m/s², got {:.4}",
        g
    );
}
...
fn test_gravity_direction_at_surface_positions() {
    ...
    for pos in positions {
        ...
        assert!(
            (dir - expected).length() < 1e-10,
            "Gravity direction at {:?} should point toward origin",
            pos
        );
    }
}
