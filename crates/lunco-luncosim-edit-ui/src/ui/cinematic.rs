...
fn report_capture_failure(commands: &mut Commands, message: impl Into<String>) {
    ...
    lunco_core::trigger_runtime_error(commands, "camera-capture-failed", message);
}
...
pub fn draw_camera_paths(
    ...
    for (path, gt) in q_paths.iter() {
        ...
        for (i, p) in path.points.iter().enumerate() {
            ...
            {
                Some(AimMode::Target(target_path)) => {
                    let mut matches = q_targets
                        .iter()
                        .filter(|(_, prim, _)| prim.path.as_str() == target_path.as_str())
                        .map(|(_, _, tgt)| tgt);
...
fn on_add_camera_here(
    ...
    let Some(root) =
        lunco_usd_bevy_stage::layer_default_prim(host.document().data()).map(|p| format!("/{p}"))
    else {
        report_capture_failure(
            &mut commands,
