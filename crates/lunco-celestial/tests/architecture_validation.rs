...
fn test_avian3d_collider_on_rotated_grid_child() {
    ...
    assert!(
        app.world().get::<Collider>(tile).is_some(),
...
fn test_body_child_rover_inherits_rotation() {
    ...
    let rover_pos_before = app
        .world()
        .get::<GlobalTransform>(rover)
        .unwrap()
        .translation();
...
fn test_gravity_direction_from_body_local_position() {
    ...
    let test_cases = [
        ...
        (
            Vec3::new(
                0.0,
                45.0_f64.to_radians().cos() as f32 * moon_radius,
    ...
    for (local_pos, expected_dir) in &test_cases {
        ...
        if *expected_dir != Vec3::ZERO {
            ...
            let angle_to_center =
                gravity_dir.angle_between(-local_pos.as_dvec3().normalize_or_zero());
            assert!(
                angle_to_center < 1e-10,
                "Gravity should point to center at 45° lat"
            );
        }
    }
...
fn test_raycaster_neg_y_error_at_various_latitudes() {
    ...
    assert!(
        neg_y_error_angle(90.0) < 1e-10,
        "North Pole should have 0° error"
    );

    // South Pole: NEG_Y = true UP → 180°
    assert!(
        (neg_y_error_angle(-90.0) - 180.0).abs() < 1e-10,
        "South Pole should have 180° error"
    );

    // Equator: NEG_Y ⟂ radial → 90°
    assert!(
        (neg_y_error_angle(0.0) - 90.0).abs() < 1e-10,
        "Equator should have 90° error"
    );

    // 45°N: 45° error
    assert!(
        (neg_y_error_angle(45.0) - 45.0).abs() < 1e-10,
        "45°N should have 45° error"
    );
