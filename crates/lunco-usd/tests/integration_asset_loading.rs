...
fn test_sandbox_scene_composes() {
    ...
    let g: [f64; 3] = view
        .value_vec3(&ground, "xformOp:scale")
        .expect("Ground scale");
    assert!(
        (g[0] - 4000.0).abs() < 1.0,
        "Ground width ~4000, got {}",
        g[0]
    );
    assert!(
        (g[1] - 0.2).abs() < 0.05,
...
fn test_rover_components_via_bevy_pipeline() {
    ...
    for (i, f) in files.iter().enumerate() {
        ...
        // Find rover entity (a rover root carries `MobilityRoot`)
        let mut q_rover = app
            .world_mut()
        ...
        let rover_ent = q_rover
            .iter(app.world())
            .next()
            .unwrap_or_else(|| panic!("{label}: No entity with MobilityRoot found"));
        ...
        assert!(
            ...
            "{label}: Mass must be ~1000, got {}",
            mass.0
        );

        let lin_damp = app
            .world()
            .get::<LinearDamping>(rover_ent)
            .unwrap_or_else(|| panic!("{label}: Missing LinearDamping"));
        assert!(
            (lin_damp.0 - 0.5).abs() < 0.1,
            "{label}: LinearDamping must be ~0.5"
        );

        let ang_damp = app
            .world()
            .get::<AngularDamping>(rover_ent)
            .unwrap_or_else(|| panic!("{label}: Missing AngularDamping"));
        assert!(
            (ang_damp.0 - 2.0).abs() < 0.1,
            "{label}: AngularDamping must be ~2.0"
        );
        ...
        for (wheel_index, w_ent) in wheel_ents.iter().enumerate() {
            ...
            // MUST have RayCaster
            assert!(
                app.world().get::<RayCaster>(*w_ent).is_some(),
            ...
            assert!(
                app.world().get::<Mesh3d>(*w_ent).is_none(),
...
fn test_rover_sim_processing_after_async_load() {
    ...
    for (f, rover_path) in rover_files {
        ...
        // mobility and networking, not an output index on an arbitrary entity.
        ...
        assert!(
            vessel_count > 0,
            "{label}: MobilityRoot + OutputPorts must be present. Got {vessel_count}."
        );
...
fn test_rover_schema_detection_after_composition() {
    ...
    for (f, rover_name) in &rover_files {
        ...
        let found = view
            .prim_paths()
            .iter()
            .any(|p| p.to_string() == format!("/{}", rover_name));
...
fn test_rover_files_have_no_baked_position() {
    ...
    for (f, rover_name) in &rover_files {
        ...
        let root_pos: Option<[f64; 3]> =
            reader.prim_attribute_value(&rover_path, "xformOp:translate");
        assert!(
            root_pos.is_none(),
            "{label}: /{} must NOT have xformOp:translate (position set by Rust), got: {:?}",
            rover_name,
            root_pos
        );
    }
...
fn test_full_scene_loads_with_rovers() {
    ...
    let mut q_rovers = app
        .world_mut()
        .query_filtered::<(Entity, &Name, &UsdPrimPath), With<MobilityRoot>>();
    ...
    let mut q_physical = app
        .world_mut()
        .query_filtered::<Entity, With<PhysicalWheel>>();
    ...
    assert!(
        rover_paths.iter().any(|p| p.contains("Skid_Raycast_1")),
        "Should have Skid_Raycast_1"
    );
    assert!(
        rover_paths.iter().any(|p| p.contains("Skid_Raycast_2")),
        "Should have Skid_Raycast_2"
    );
    assert!(
        rover_paths.iter().any(|p| p.contains("Skid_Physical_1")),
        "Should have Skid_Physical_1"
    );
    assert!(
        rover_paths
            .iter()
            .any(|p| p.contains("Ackermann_Raycast_1")),
        "Should have Ackermann_Raycast_1"
    );
    assert!(
        rover_paths
            .iter()
            .any(|p| p.contains("Ackermann_Physical_1")),
...
fn test_valentine_color_override() {
    ...
    assert!(
        view.has_prim(&chassis),
        "Skid_Raycast_1/Chassis prim must exist"
    );
    ...
    assert!(
        (display_color[0] - 0.85).abs() < 0.01,
        "Red should be 0.85, got {}",
        display_color[0]
    );
    assert!(
        (display_color[1] - 0.15).abs() < 0.01,
        "Green should be 0.15, got {}",
        display_color[1]
    );
    assert!(
        (display_color[2] - 0.12).abs() < 0.01,
        "Blue should be 0.12, got {}",
        display_color[2]
    );
}
