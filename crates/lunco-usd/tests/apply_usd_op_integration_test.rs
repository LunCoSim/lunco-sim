...
fn test_apply_usd_op_integration() {
    ...
    app.init_asset::<Mesh>();
    // No `StandardMaterial` registry: after the render decoupling this crate does not link
    // `bevy_pbr` at all. The USD prim processor states a `PbrLook` intent and
    // `lunco-render-bevy` binds the GPU material — nothing here needs the asset type.
