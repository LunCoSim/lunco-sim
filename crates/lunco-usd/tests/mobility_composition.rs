...
fn a_wheel_composes_its_tire() {
    ...
    assert_eq!(
        view.real(&fl, "physics:dynamicFriction"),
        Some(1.5),
...
fn every_rover_wheel_satisfies_the_unified_param_reader() {
    ...
    for rover in rovers {
        ...
        for p in view.prim_paths() {
            ...
            // runtime topology scan before it calls the reader.
...
/// The live-resync CLAIM must be prim-scoped: `physxVehicleWheel:mass` on a WHEEL routes
...
#[test]
fn six_wheel_camera_composes_its_rear_mount_offset() {
    ...
    assert_eq!(
        ...
        Some([0.0, 0.55, 4.0]),
        "the six-wheel asset must keep its rear-mounted camera eye height"
    );
    assert_eq!(
