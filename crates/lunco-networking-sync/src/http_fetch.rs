...
pub fn fetch_missing_assets_http(
    ...
    // `asset_base_url` is host-advertised over the authenticated session. The
    // HTTP bytes are still verified by CID before they enter the cache; the
    // URL is a deployment-trusted transport endpoint, not an asset authority.
...
async fn fetch_bytes_web(
    ...
    lunco_assets_runtime::web_fetch::fetch_bytes_cached("lunco-scenario-assets-v1", url, settings)
        .await
}
