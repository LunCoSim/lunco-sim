...
impl WheelAttachmentTopology {
    ...
    /// Iterate over valid wheel bindings in the composed stage.
    pub fn bindings(&self) -> impl Iterator<Item = (&String, &WheelAttachmentBinding)> {
        self.bindings.iter()
    }

    /// Iterate over wheel paths rejected by the authored topology resolver.
