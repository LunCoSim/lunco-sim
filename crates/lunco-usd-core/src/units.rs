...
//! colliders, `lunco-luncosim-edit-ui`'s gizmo — already funnels through these:
//!
//! | decoder | conversion |
//! |---|---|
...
impl StageMetrics {
    /// Read stage metrics from a composed-reader adapter.
    ///
    ...
    pub fn from_reader<R: StageMetadataReader + ?Sized>(
        reader: &R,
    ) -> Result<Self, StageMetricsError> {
...
/// Minimal metadata seam needed to interpret USD stage conventions.
///
/// The core does not depend on the runtime composed-reader trait. The Bevy USD
/// integration implements this adapter for its live and prepared readers.
pub trait StageMetadataReader {
    /// Return the composed value of a stage metadata field.
