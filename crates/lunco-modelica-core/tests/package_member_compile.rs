...
fn lander_compiles_without_a_dead_logic_dependency() {
    ...
    let result = compiler.compile_str(
        "Lander",
        lunco_modelica_core::models::get_model("Lander.mo").expect("bundled Lander.mo"),
...
fn stripped_lander_compiles_without_a_dead_logic_dependency() {
    let source = lunco_modelica_core::models::get_model("Lander.mo").expect("bundled Lander.mo");
...
fn standalone_model_still_compiles_via_user_overlay() {
    let balloon = lunco_modelica_core::models::get_model("Balloon.mo").expect("bundled Balloon.mo");
    let mut compiler = ModelicaCompiler::new();
