...
//!
//! Every structured op below produces a **minimal text splice** via
//! `ast_mut::class_patch` / `ast_mut::document_patch`: it rewrites the bytes it
//! owns and copies the rest of the class through untouched. Nothing here may
//! round-trip a class or a document through rumoca's emitter — that is what used
//! to turn a canvas drag into silent edits of unrelated declarations. See
//! `crates/lunco-modelica-core/src/ast_mut/edit.rs`.
