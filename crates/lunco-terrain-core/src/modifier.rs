...
/// * preserves the composed DEM exactly at every sampled point;
/// * applies no extrapolated apron, cap, wall, or boundary relief.
///
...
/// The body radius and datum are used only to compute the physical site sphere;
/// the authored elevation itself remains untouched.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct BodyCurvature {
    /// Body mean radius (metres).
    pub radius_m: f64,
    /// Site elevation (metres, body datum), used to obtain the local sphere
    /// radius. Zero for a DEM already expressed relative to the body radius.
    pub datum_m: f64,
}

impl BodyCurvature {
    pub fn new(radius_m: f64, datum_m: f64) -> Self {
        Self { radius_m, datum_m }
    }
}
...
mod tests {
    ...
    #[test]
    fn body_curvature_preserves_dem_and_hugs_sphere() {
        let r = 1.737e6;
        let c = BodyCurvature::new(r, 0.0);
        // Site centre: untouched (full relief, zero sagitta).
        assert!((c.apply(0.0, 0.0, 120.0) - 120.0).abs() < 1e-9);
        // Relief is kept, with only the physical sagitta subtracted.
        let d = 4000.0;
        ...
        let outside = 20_000.0;
        let sag_o = (r * r - outside * outside).sqrt() - r;
        assert!((c.apply(outside, 0.0, 300.0) - (300.0 + sag_o)).abs() < 1e-6);
    }

    /// An absolute-datum DEM (Hadley reads ≈ −1918 m at every point) must keep
    /// its authored elevation exactly; curvature must not move it toward zero.
    #[test]
    fn body_curvature_preserves_absolute_datum() {
        let (r, datum) = (1.737e6, -1918.0);
        let c = BodyCurvature::new(r, datum);
        assert!((c.apply(0.0, 0.0, -1947.6) - -1947.6).abs() < 1e-6);
        let local_radius = r + datum;
        let sag = (local_radius * local_radius - 2.0 * 700.0f64.powi(2)).sqrt() - local_radius;
        assert!((c.apply(-700.0, -700.0, -1800.0) - (-1800.0 + sag)).abs() < 1e-6);
    }
