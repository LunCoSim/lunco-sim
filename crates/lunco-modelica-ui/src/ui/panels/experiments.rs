...
impl ExperimentsPanel {
    ...
    fn render_setup_section(&self, ui: &mut egui::Ui, ctx: &mut PanelCtx) {
            ...
            .and_then(|r| r.host(doc))
        {
            Some(h) => {
                ...
                match crate::ui::context::default_simulation_class(ctx, doc) {
                    Some(c) => (c, candidates),
                    None => return,
                }
            }
    ...
    fn render_override_editor(&self, ui: &mut egui::Ui, ctx: &mut PanelCtx) {
        ...
        let drilled = crate::ui::context::drilled_class_for_doc(ctx, doc);
...
pub fn has_experiment_runs(ctx: &mut PanelCtx) -> bool {
    ...
    let live_exists = crate::ui::context::simulator_for(ctx, doc_id).is_some();
    if live_exists {
        return true;
    }
    let twin = crate::ui::doc_pin::twin_id_for_doc(doc_id);
