...
//! `.rhai` guided script puts instructions on screen with no Rust:
//!
//! ```rhai
...
/// The owning guided decides what Continue means; the workbench only presents
/// the recovery action and emits the typed request.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct GuidedRecovery {
...
/// The authored guided target could not be resolved in the current workbench
/// presentation. The guided launcher owns the lifecycle response; the overlay
...
/// One coach-mark step of a guided tour (see [`GuidedOverlay::tour`]).
...
pub struct GuidedStopRequested;

/// Continue the current lesson after dismissing a recoverable presentation
/// problem. The guided launcher advances a guided step when one is active.
#[derive(Event, Clone, Copy, Debug, Default)]
pub struct GuidedRecoveryContinueRequested;

/// Retry resolving the current authored presentation target.
#[derive(Event, Clone, Copy, Debug, Default)]
pub struct GuidedRecoveryRetryRequested;
...
#[Command(default)]
pub struct GuidedNext {}

/// Return to the previous guided guided step.
#[Command(default)]
pub struct GuidedBack {}
...
fn register_guided_navigation(app: &mut App) {
    app.register_type::<GuidedNext>()
        .register_type::<GuidedBack>()
        .register_type::<GuidedSkip>()
        .add_observer(on_guided_next)
        .add_observer(on_guided_back)
        .add_observer(on_guided_skip);
}
...
/// authored guided target. It owns input until the learner chooses an action,
/// while leaving the lesson host, simulation, and loaded scene alive.
fn draw_guided_recovery(
    mut egui_ctx: EguiContexts,
    hud: Res<GuidedOverlay>,
    theme: Option<Res<lunco_theme::Theme>>,
    ...
    egui::Area::new(egui::Id::new("lunco_guided_recovery_scrim"))
        .order(GUIDED_SCRIM_ORDER)
        .interactable(true)
    ...
    let mut retry = false;
    let mut stop = false;
    egui::Area::new(egui::Id::new("lunco_guided_recovery_card"))
        .order(GUIDED_OVERLAY_ORDER)
        .interactable(true)
        .fixed_pos(card_pos)
        .show(ctx, |ui| {
    ...
    if continue_lesson {
        commands.trigger(GuidedRecoveryContinueRequested);
    }
    if retry {
        commands.trigger(GuidedRecoveryRetryRequested);
    }
    if stop {
        commands.trigger(GuidedStopRequested);
    }
}
...
fn report_missing_anchor(hud: &mut GuidedOverlay, commands: &mut Commands, anchor: &str) {
    if hud.reported_missing_anchor.as_deref() == Some(anchor) {
        return;
    }
    hud.reported_missing_anchor = Some(anchor.to_owned());
    commands.trigger(GuidedTargetUnavailable {
...
fn draw_spotlight(
    ...
    let Ok(ctx) = egui_ctx.ctx_mut() else { return };
    let Some(screen) = guided_content_rect(ctx, &anchors) else {
        return;
    };
    let theme = theme
        .map(|t| t.clone())
    ...
    let target = if key.is_empty() {
        ...
        guided_anchor_rect(&anchors, &key, screen)
    };
...
/// controls. Controls fire `cmd:Guided{Next,Back,Skip,Goto}` on the bus; the
...
fn draw_tour(
    ...
    let target = if step.anchor.is_empty() {
        ...
        guided_anchor_rect(&anchors, &step.anchor, screen)
    };
    ...
    // scrim. Paint their ring in the guided layer while leaving the title bar
    // itself interactive.
    if let (Some(target), None) = (target, target_in_content) {
        let painter = ctx.layer_painter(egui::LayerId::new(
            GUIDED_OVERLAY_ORDER,
    ...
    if next {
        commands.trigger(GuidedNext {});
    }
    if back {
        commands.trigger(GuidedBack {});
    }
    if skip {
        commands.trigger(GuidedSkip {});
    }
    if stop {
        commands.trigger(GuidedStopRequested);
    }
