//! Generate the LunCoSim desktop icon from the tracked platform SVG when the
//! native package feature is enabled.
//!
//! The icon is presentation and packaging output. Ordinary UI builds do not
//! compile the SVG/image toolchain or emit an icon; `scripts/build_native.sh`
//! enables `package-icons` for the packaged `luncosim` binary.
            ...
            .map(|size| {
                ...
                IcoFrame::as_png(pixmap.data(), size, size, ExtendedColorType::Rgba8)
                    .unwrap_or_else(|error| {
                        panic!("failed to encode {size}px LunCoSim icon: {error}")
                    })
            })
            .collect();
...
fn main() {
    if std::env::var_os("CARGO_FEATURE_PACKAGE_ICONS").is_some() {
        #[cfg(feature = "package-icons")]
        package_icons::run();
    }
}
