...
//!
//! It was an enum (`SolverChoice`, four variants) with a single mapping function
//! documented as "the ONLY place a solver is chosen". The live co-simulation path
//! did not call it: `worker::live_stepper_options` hardcoded the explicit RK
//! family for **every** live model, so the two paths silently disagreed.
//!
//! MEASURED CONSEQUENCE: a rover's generated electrical island (battery + solar
//! panel) is an implicit system. Handed to the explicit stepper it dies with
//! `rk45 backend only supports a narrow explicit ODE subset: algebraic refresh
//! row 2 cannot be solved`, publishes no ports, and reports nothing a driver
//! would see. Every solar rover in the repo was dead, and the shipped demo scene
//! with it.
//!
//! A closed enum is what made the shortcut attractive: adding or choosing a
//! solver meant editing the enum, the mapping, the parser and the UI list, so a
//! caller with a special case wrote a constant instead of participating. Here a
//! solver is registered data, and **no caller names one** — callers state what
//! the model needs and what the runtime demands, and [`resolve`] answers.
//!
//! ## The two facts selection is made of
//!
//! * [`ModelCaps`] — what the MODEL requires. Derived from the compiled DAE
//!   (`algebraics` non-empty ⇒ implicit), never authored, never guessed. The
//!   thing that broke was a guess.
...
//! example, and it was getting the predicted class's stepper.
//!
...
impl From<&str> for SolverId {
    /// Normalizes the way the old `FromStr` did — case, `-`, `_`, spaces — so
    /// `"TR-BDF2"`, `"tr_bdf2"` and `"trbdf2"` are one id and cannot register
    /// twice under three spellings.
    fn from(s: &str) -> Self {
...
pub struct SolverRequest {
    ...
    /// An explicit choice (`lunco:program:solver`, the experiments UI, the API).
    /// Honoured when capable and REFUSED when not — never silently replaced,
    /// which is how the old code turned a typo into BDF.
...
/// 1. An implicit model needs an implicit-capable solver. This is the one that
///    was being violated silently on every solar rover.
...
mod tests {
    ...
    /// The regression, stated as a unit: an implicit model must never be handed
    /// the explicit family. This is the whole solar bug in four lines.
    #[test]
    ...
    /// swapped. The old parser degraded unknown strings to BDF; degrading a
    /// *known but wrong* one would be the same class of silence.
    #[test]
    ...
    /// An implicit model with nothing implicit registered must ERROR, not take
    /// the explicit solver anyway. This is the case that shipped dead: the
    /// stepper accepted the model and failed later, per-step, in a log line.
    #[test]
