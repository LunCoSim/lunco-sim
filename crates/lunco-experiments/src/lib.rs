...
pub struct RunBounds {
    ...
    /// Integration method override, by registered id. `None` = let
    /// [`solver::resolve`] pick from the model's and the runtime's needs, which
    /// is the normal case; `Some` is an explicit demand that is REFUSED if the
    /// solver cannot serve the model rather than silently replaced.
