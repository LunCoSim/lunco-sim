...
/// Bounded set of recently-applied `(origin, op_id)` pairs for idempotent replay
/// rejection. Keyed on the pair — not `op_id` alone — because `OpId`s are minted
/// per-process from a non-unique seed, so two distinct clients can independently
/// mint the same `op_id` in the same window; keying on the originating session
/// keeps each peer's command stream from colliding with another's.
#[derive(Resource)]
...
impl SyncDedup {
    /// `true` if `(origin, op)` is new (apply it); `false` if already seen (drop).
