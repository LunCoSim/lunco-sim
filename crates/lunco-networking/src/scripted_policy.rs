...
//! **drive kernels** ([`lunco_mobility::ScriptedDriveKernel`]). But a scripted
...
pub enum PolicyKind {
    ...
    Authorize,
    /// A vehicle drive kernel ([`lunco_mobility::ScriptedDriveKernel`] by hook id).
    DriveKernel,
}
...
/// `DriveKernel` just becomes available to [`lunco_mobility::ScriptedDriveKernel`]
/// components by id. Re-registering hot-replaces (idempotent for a stable source).
