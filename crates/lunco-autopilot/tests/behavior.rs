...
fn json_tree_drives_and_sequences_waypoints() {
    ...
    assert!(
        ctx.out.0 > 0.0,
        "after wp1, drives toward wp2, got {:?}",
        ctx.out
    );
    assert!(
        ctx.out.1.abs() > 0.0,
        "should steer toward wp2 (+Z), got {:?}",
        ctx.out
    );
}
...
fn patrol_loops_waypoints_and_dwells_at_each() {
    ...
    assert!(
        braked >= 2,
        "patrol should dwell (brake) at waypoints; braked {braked} ticks"
    );
}
...
fn wait_latches_a_mission_time_deadline_and_resets_across_repeats() {
    ...
    assert_eq!(
        rep.0.tick(&mut ctx),
        Status::Running,
        "lap 1 armed at t=0 (deadline 1)"
    );
    ctx.now = 1.0;
    assert_eq!(
        rep.0.tick(&mut ctx),
        Status::Running,
        "lap 1 done at t=1; child cleared"
    );
    ctx.now = 2.0;
    assert_eq!(
        rep.0.tick(&mut ctx),
        Status::Running,
        "lap 2 re-arms at t=2 (deadline 3)"
    );
    ctx.now = 3.0;
    assert_eq!(
        rep.0.tick(&mut ctx),
        Status::Success,
        "lap 2 done at t=3 → repeat succeeds"
    );
}
...
fn face_pivots_in_place_then_succeeds_when_aligned() {
    ...
    assert!(
        ctx.out.1.abs() > 0.0,
        "face steers toward the target, got {:?}",
        ctx.out
    );
...
fn invert_negates_the_arrived_condition() {
    use lunco_behavior::Status;
    let mut inv = AutopilotBehavior::from_json(
        r#"{"kind":"invert","child":{"kind":"arrived","target":[0.0,0.0,0.0],"radius":2.0}}"#,
    )
    .unwrap();
...
fn force_and_retry_decorators_map_and_re_attempt() {
    ...
    let mut fs =
        AutopilotBehavior::from_json(r#"{"kind":"force_success","child":{"kind":"fail"}}"#)
            .unwrap();
    assert_eq!(fs.0.tick(&mut ctx), Status::Success);
    let mut ff =
        AutopilotBehavior::from_json(r#"{"kind":"force_failure","child":{"kind":"succeed"}}"#)
            .unwrap();
    assert_eq!(ff.0.tick(&mut ctx), Status::Failure);
    // retry(times:2, fail): 1st failure retries (Running), 2nd exhausts → Failure.
    let mut rt =
        AutopilotBehavior::from_json(r#"{"kind":"retry","times":2,"child":{"kind":"fail"}}"#)
            .unwrap();
...
fn follow_tracks_a_live_target_and_fails_when_it_vanishes() {
    ...
    let mut behavior =
        AutopilotBehavior::from_json(r#"{"kind":"follow","target":42,"speed":0.6,"radius":3.0}"#)
            .unwrap();
    ...
    assert_eq!(
        behavior.0.tick(&mut ctx),
        Status::Running,
        "keeps following, never finishes"
    );
    assert!(
        ctx.out.0 > 0.0,
        "drives toward the live target, got {:?}",
        ctx.out
    );
    ...
    assert!(
        ctx.out.2 > 0.5,
        "holds station (brakes) within radius, got {:?}",
        ctx.out
    );
    ...
    assert!(
        ctx.out.2 > 0.5,
        "brakes when the target is lost, got {:?}",
        ctx.out
    );
}
...
fn intercept_leads_a_moving_target_and_succeeds_on_contact() {
    ...
    assert!(
        ctx.out.0 > 0.0,
        "drives toward the lead point, got {:?}",
        ctx.out
    );
    ...
    assert!(
        ctx.out.1.abs() > 0.0,
        "leads the target (steers off the tail), got {:?}",
        ctx.out
    );
...
fn obstacle_ahead_senses_a_vessel_in_the_forward_cone_and_excludes_self() {
    use lunco_behavior::Status;
    let mut behavior =
        AutopilotBehavior::from_json(r#"{"kind":"obstacle_ahead","distance":6.0,"cone":60.0}"#)
            .unwrap();
...
fn facing_guards_heading_and_hold_stays_running() {
    ...
    let mut facing = AutopilotBehavior::from_json(
        r#"{"kind":"facing","target":[10.0,0.0,0.0],"tolerance":8.0}"#,
    )
    .unwrap();
...
fn cooldown_blocks_re_entry_for_the_lockout_window() {
    ...
    let mut behavior = AutopilotBehavior::from_json(
        r#"{"kind":"cooldown","seconds":2.0,"child":{"kind":"succeed"}}"#,
    )
    .unwrap();
    let mut ctx = DriveCtx {
        self_gid: 0,
    ...
    assert_eq!(
        behavior.0.tick(&mut ctx),
        Status::Failure,
        "within lockout → blocked"
    );
    ctx.now = 2.0;
    assert_eq!(
        behavior.0.tick(&mut ctx),
        Status::Success,
        "lockout elapsed → fires again"
    );
}
...
fn steer_clear_goes_straight_when_open_and_turns_toward_the_open_side() {
    use lunco_behavior::Status;
    let mut behavior =
        AutopilotBehavior::from_json(r#"{"kind":"steer_clear","speed":0.6}"#).unwrap();
    ...
    assert_eq!(behavior.0.tick(&mut ctx), Status::Running);
    assert!(
        ctx.out.0 > 0.0 && ctx.out.1.abs() < 1e-6,
        "open → straight, got {:?}",
        ctx.out
    );
    ...
    assert!(
        ctx.out.1.abs() > 0.0,
        "blocked → steers toward open side, got {:?}",
        ctx.out
    );
    assert!(
        ctx.out.0 > 0.0 && ctx.out.0 < 0.6,
        "throttle eased when tight, got {:?}",
        ctx.out
    );
