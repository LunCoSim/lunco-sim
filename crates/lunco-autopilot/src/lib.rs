...
fn on_disengage_autopilot(
    ...
    match q.iter_mut().find(|(_, ap)| ap.vessel == cmd.vessel) {
        Some((entity, mut ap)) => {
            ...
            ap.disengage();
            // The shared ReleaseControl observer fences stale SetPorts writes,
            // clears every authored command input (including lander/RCS ports),
            // and leaves the safe neutral state in place for the next owner.
    ...
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            ...
            (
                ...
                // wire propagation carries a source `Port` to its target;
                // otherwise its commands are picked up in this tick or the next
                // depending on the schedule's parallel layout, which desyncs
                // prediction replay.
