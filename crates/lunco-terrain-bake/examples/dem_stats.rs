...
fn main() {
    let path = std::env::args().nth(1).expect("usage: dem_stats <path.tif>");
    ...
    println!("raster {w}x{h}  valid {valid}/{total} ({:.1}%)", 100.0 * valid as f64 / total as f64);
    ...
    println!("measured bbox: x {x0}..={x1}  y {y0}..={y1}  ({} x {})", x1 - x0 + 1, y1 - y0 + 1);
    ...
    for y in (0..h).step_by((h / 24).max(1)) {
        ...
        println!("  y{y:>6} {:>5.1}% {}", 100.0 * n as f64 / w as f64, bar(n as f64 / w as f64));
    }
    println!("\ncol validity (every {}th):", w / 24);
    for x in (0..w).step_by((w / 24).max(1)) {
        let n = (0..h).filter(|&y| heights[y * w + x].is_finite()).count();
        println!("  x{x:>6} {:>5.1}% {}", 100.0 * n as f64 / h as f64, bar(n as f64 / h as f64));
    }
}
