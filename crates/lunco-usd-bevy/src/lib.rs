...
pub use light::{read_intensity_with_exposure, LightReadError};
pub mod author;
pub mod camera_path;
pub mod canonical;
pub mod curve_sweep;
...
fn instantiate_usd_prim(
    ...
    let Some(cs) = canonical.get(id) else {
        warn!(
            "[canonical] no live stage for {} — skipping visual instantiate",
            prim_path.path
        );
        return;
    };
...
fn read_scalar_f32_at(
    ...
    reader.real_f32_at(path, attr, time).or_else(|| {
