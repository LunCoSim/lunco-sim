...
//! through the normal scene lifecycle command. Starting a UI-only lesson after
//! a world-owning lesson clears that outgoing scene before the UI-only host
//! starts. A lesson may still use
...
pub struct TrackMeta {
    ...
    pub label: String,
    /// Authored presentation perspective, if this track declares one.
...
/// can clear it through the normal scene lifecycle. A UI-only lesson has no
/// world of its own; starting it after a world lesson still clears the outgoing
/// scene before its UI-only host starts.
#[derive(Resource, Default)]
...
fn on_start_tutorial(trigger: On<StartTutorial>, mut commands: Commands) {
    ...
    commands.queue(move |world: &mut World| {
        ...
        // replacement lesson must release the outgoing lesson's scene ownership
        // before the new host starts. A UI-only lesson has no replacement world,
        // but it must still clear an outgoing tutorial scene rather than leaving
        // that scene running underneath a lesson with no visual payload.
        ...
        let outgoing_scene = outgoing_world.is_some()
            || world
        ...
        if let Some(scene) = meta.world.clone() {
            ...
            let clear_before_load = outgoing_scene;
...
fn on_skip_tutorial(_t: On<SkipTutorial>, mut commands: Commands) {
    commands.queue(|world: &mut World| {
        ...
        if clear_world {
            ...
            // command. A UI-only tour has no owned scene after its launch clear,
...
fn on_tutorial_target_unavailable(
    ...
    let detail = format!(
        "tutorial '{}' requires UI anchor '{}' but the composed workbench did not publish it",
        id,
...
fn on_scene_transition_started(
    ...
    let belongs_to_pending = match &trigger.event().transition {
        lunco_core::SceneTransition::Load { path, .. } => pending
            .0
            .as_ref()
            .and_then(|request| request.world.as_deref())
...
fn on_scene_transition_completed(
    ...
    match &trigger.event().transition {
        lunco_core::SceneTransition::Clear => {
            ...
            let Some(scene) = request.world.clone() else {
                commands.queue(|world: &mut World| {
                    let should_start = {
                        ...
                        if !request.clear_before_load || request.world.is_some() {
                            return;
                        }
                    ...
                    let request = world
                        .resource_mut::<PendingTutorialStart>()
                        .0
                        .take()
                        .expect("pending UI-only tutorial request exists");
                    start_tutorial_scenario(world, request.id, request.source, None);
                });
                return;
            };
            commands.queue(move |world: &mut World| {
                let should_load = {
                    ...
                    if !request.clear_before_load
                        || request.world.as_deref() != Some(scene.as_str())
                    {
                        return;
                    }
                    request.clear_before_load = false;
...
fn on_scene_transition_failed(
    ...
    let matches_pending = path.is_some_and(|path| {
        pending_start
            .0
            .as_ref()
            .is_some_and(|request| request.world.as_deref() == Some(path))
    });
    if path.is_none()
        && pending_start
            .0
...
mod tests {
    ...
    #[test]
    fn switching_from_world_lesson_to_ui_lesson_clears_before_start() {
        #[derive(Resource, Default)]
        struct TransitionsSeen(Vec<lunco_core::SceneTransitionRequest>);
        ...
        for (id, world) in [
            ...
            app.register_tutorial(TutorialMeta {
                id: id.into(),
                title: id.trim_start_matches("/Test/").into(),
                blurb: String::new(),
                app: "/Test".into(),
                difficulty: String::new(),
                format: curriculum::LessonFormat::Tour,
                script: "lunco://tutorials/sandbox/sandbox_intro.rhai".into(),
    ...
    #[test]
    fn ui_only_tutorial_clears_an_existing_scene_without_tutorial_owner() {
        #[derive(Resource, Default)]
        struct TransitionsSeen(Vec<lunco_core::SceneTransitionRequest>);
