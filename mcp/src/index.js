...
const STATIC_TOOLS = [
  ...
  {
    name: 'run_scenario',
    description: 'Attach a persistent rhai scenario to an entity at runtime — the scenario-loading entry point. Registers `source` as a ScriptDocument and attaches a `ScriptedModel{Rhai}` to `target`, so the per-entity runtime builds a native `task(self)` tree, evaluates optional `mission(self)` objectives, and runs optional lifecycle/event hooks (`self` is the host entity api_id). The script can READ world state (`world_pos(id)`, `world_forward(id)`, `get(id,"Comp.field")`, `find(name)`, `sim_tick()`), DRIVE the sim via the generic `cmd("CommandName", #{...})` bridge over every registered command (or prelude helpers `drive`/`brake`/`nav_to`), and EMIT/RECEIVE events (`emit(name,value)` → TelemetryEvent bus; task `wait_for` leaves are fed by the kernel, while `on_event` receives external events next tick). Pass `params` (a JSON object) to parameterize a reusable scenario — the script reads it as the read-only `params` constant (e.g. `params.speed`), so one source serves many entities. Idempotent + HOT-RELOAD: re-running on the same entity recompiles in place (bumps the document generation), so this also serves as live scenario editing. Returns `{document_id, generation}`. Pass entity IDs from `list_entities`.',
    inputSchema: {
      type: 'object',
      properties: {
        target: { type: 'string', description: 'Entity api_id to attach the scenario to (from list_entities).' },
        source: { type: 'string', description: 'rhai scenario source. Define `fn task(me) { ... }` for production progression, optionally `fn mission(me)`, `on_start`, `on_event`, and `on_stop`. Prelude helpers include task-tree constructors, nav_to, drive, brake, distance, arrived, emit, plus the host verbs cmd/world_pos/world_forward/get/find/sim_tick.' },
