#!/usr/bin/env bash
# ============================================================================
# LunCoSim — luncosim deploy (server binary + wasm client)
# ============================================================================
...
#     --web <path>     remote dir for the wasm client bundle      (dist/luncosim/)
#
# Options:
...
#
#     # server only:
#     ./scripts/deploy_server.sh deploy@host --server ~/luncosim-server
#
#     # server + web in one go:
#     ./scripts/deploy_server.sh deploy@host \
#         --server ~/luncosim-server --web /var/www/html/sandbox.lunco.space
# ============================================================================
set -euo pipefail
...
WEB_DIST="$PROJECT_DIR/dist/luncosim"

if [ ! -x "$SERVER_DIST/luncosim" ] || [ ! -d "$SERVER_DIST/assets" ]; then
    error "no server bundle at $SERVER_DIST (missing luncosim binary or assets/)"
...
if [ -n "$WEB_PATH" ] && [ ! -d "$WEB_DIST" ]; then
    ...
    info  "build it first:  ./scripts/build_web.sh build luncosim --release"
    exit 1
fi
...
info "host    : $HOST"
info "server  : $SERVER_DIST/luncosim  →  $HOST:$SERVER_PATH  ($(du -sh "$SERVER_DIST/luncosim" | cut -f1))"
