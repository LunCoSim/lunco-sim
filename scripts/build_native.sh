...
# AppImage/AppDir). Velopack owns the generated Linux desktop entry from the
# package identity, so the staging directory must not carry a second desktop
# file. A package must fail closed if the native output is missing.
stage_app_icons() {
...
if [ "$BINARY" = "luncosim" ]; then
    PACKAGE_FEATURE_ARGS+=(--features package-icons,updates)
fi
    ...
    --bin "$BINARY" -p "$CRATE" \
    "${PACKAGE_FEATURE_ARGS[@]+"${PACKAGE_FEATURE_ARGS[@]}"}" \
