...
| **USD integration** | OpenUSD↔Bevy: headless document/core, geometry, visuals, physics, sim schemas, actuation, materials | `lunco-usd-core`, `lunco-usd-geometry`, `lunco-usd`, `lunco-usd-bevy`, `lunco-usd-avian`, `lunco-usd-actuation`, `lunco-materials` |
| **Networking & API** | replication, HTTP API, telemetry, attributes | `lunco-networking`, `lunco-api`, `lunco-telemetry` |
| **Workbench & UI** | IDE shell, shell-independent widgets, reusable Twin/Files browser, viz, 2D canvas, edit tools, render intent/recovery, web boot | `lunco-workbench`, `lunco-workbench-core`, `lunco-workbench-widgets`, `lunco-workbench-browser`, `lunco-ui`, `lunco-viz`, `lunco-canvas`, `lunco-luncosim-edit-core`, `lunco-luncosim-edit-ui`, `lunco-render-recovery` |
...
- Composition roots: `lunco-luncosim-core` owns the headless-safe runtime; `lunco-luncosim` composes it with `lunco-luncosim-ui` for the GUI; `lunco-luncosim-server` launches core directly. USD stage composition is owned by `lunco-usd-bevy::flatten_stage`.
