...
**already-running** workbench, never `pkill`, and always send `"params":{}` —
top-level arguments outside `params` are silently dropped and the command runs
with defaults (a camera command then quietly aims at the origin).
