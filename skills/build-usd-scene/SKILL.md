...
- **Celestial time is an explicit scene choice.** A `SolarSystem` reference makes body poses ephemeris-driven; `LunCoEpochAPI` makes the scene responsible for choosing the ephemeris epoch. Apply it to the scene root and author `lunco:time:epochJd` there. If the lesson needs repeatable light but not astronomy, use the fixed `DistantLight` contract instead.
