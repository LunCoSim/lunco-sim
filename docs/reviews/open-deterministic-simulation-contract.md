...
| D17 | P1 | Telemetry sampling is an exclusive `FixedUpdate` system that walks the cached channel list for due checks, then triggers retention, API-subscription, and logging observers inline ([`telemetry/src/lib.rs`](../../crates/lunco-telemetry/src/lib.rs), [`telemetry-core/src/log.rs`](../../crates/lunco-telemetry-core/src/log.rs)). Keep tick-stamped capture bounded and due-driven; move non-authoritative fan-out, formatting, and I/O out of physics, and prove telemetry load does not reduce tick throughput. | Open |
