<!-- AUTO-GENERATED by `cargo run --manifest-path tools/gen-command-docs/Cargo.toml`.
...
**158 commands** across **21** crates. 24 command(s) below lack a `///` description — marked _(no description)_; add a doc comment on the struct to fix it.
...
- [`lunco-terrain-surface`](#lunco-terrain-surface) (`lunco-terrain-surface`, 6 commands)
...
- [`lunco-api`](#lunco-api) (`lunco-api`, 1 command)
...
- *fields:* none — call with `CaptureScreenshot` (no params)
...
- *defined in:* `crates/lunco-api/src/executor.rs`
...
<!-- scanned 599 .rs files across `crates/`; 0 parse failure(s) skipped -->
