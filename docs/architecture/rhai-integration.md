...
`norm_squared`, `vnorm`, `qrot`, quaternion multiplication/inversion, and
XYZ-degree Euler conversion) execute in Rust/glam and validate finite,
...
`timestamp` is only the derived TDB epoch label. While the scene-readiness
lifecycle gate is closed, events are not buffered because no scenario can
...
fn possess(r)           { cmd("AcquireControl", #{ target: r }); }
