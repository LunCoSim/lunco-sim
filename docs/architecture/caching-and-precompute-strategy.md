...
| **Single asset-resolution owner** | `lunco-assets/src/asset_sources.rs` `register_lunco_asset_sources` | All schemes (`lunco://`, `twin://`, `cached_textures://`) registered in ONE place before `AssetPlugin`, with every URI↔location mapping (`local_path`, `twin_uri`, `library_rel`, `id_to_disk_path`) exported from this crate so no consumer re-derives one. A `precompute://` reader would be added here. |
