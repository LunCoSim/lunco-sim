...
| Generic component/reference construction, typed parameter plans, and invalid identity rejection | `assembly_component_builder` | same Rhai scenario's invalid asset, missing parent, duplicate parameter, and occupied identity cases | dynamic `assembly_builder`/`assembly_edit` plans, proposal/review/commit, and composed `QueryUsdPrim` |
