...
| Layer | Owns | Where |
|---|---|---|
| **Kernel** (Rust) | The pairwise sweep, the cadence, and the GEOMETRY: range, local elevation, analytic body occlusion, terrain occlusion. | `lunco-celestial/src/link.rs` |
...
LinkPeer  { peer, connected, range_m, elevation_deg }
```

`class` is an authored role string. **The core never interprets it** — it is passed
through to the verdict/routing policy and used as the node's key. (This is deliberate:
...
over the body registry) → terrain occlusion. It then publishes `LinkState` and emits
`link.aos` / `link.los` telemetry events on the rising/falling edges.
...
ctx:     a, b, class_a, class_b, range_m, elev_a, elev_b,
         min_elev_a, min_elev_b, occluded, occluded_by,
         terrain_blocked, max_range_m
returns: bool
```
...
(range ∧ elevation masks ∧ ¬occluded ∧ ¬terrain_blocked).

This is why terrain occlusion in Rust does **not** make the feature un-scriptable: the
kernel computes the `terrain_blocked` *fact*; the script decides what it *means*.
...
```
query("Links")  ->  { nodes: [key], adj: { key: [peer…] }, edges: [{a,b,range_m}] }
```
...
link_path(from, to)      // the shortest hop path, as node keys
can_reach(from, station) // "can this rover talk home?"
```

Node keys are the authored `class` (else the prim `Name`). Tune or replace any of this
with no core rebuild.
...
def Xform "Relay" (
    ...
    custom bool   lunco:linkNode              = 1
    custom string lunco:link:class            = "relay"
    custom double lunco:link:maxRangeM        = 2e7
    custom double lunco:link:minElevationDeg  = 5.0
    custom int    lunco:orbit:body            = 301      # a KeplerOrbit relay
}
```
...
Smoke scene: `assets/scenes/sandbox/link_test.usda`.
