...
### Separate rover radio graph

Rover-to-rover communication is not a direct `LinkState` edge. The kernel also
...
`LunCoLinkAPI` / `LunCoOccluderAPI` are declared in `lunco-usd-authoring/schema/schema.usda`, so
