...
- `UsdOp` plus `ApplyUsdOp` is the only write path. It supplies journaling,
...
| Connection canvas | Project typed `inputs:`/`outputs:` and physics joint relationships; write `SetConnection` through `ApplyUsdOp` |
...
4. Apply the complete operation set through `ApplyUsdOp`.
...
4. Validate the complete user path against the production binary: open a Twin,
   attach or snap a component, inspect the composed joint and ports, save, reload,
   and confirm the body remains stable.
