...
| Propulsion-derived engine plume | Modelica propulsion/nozzle outputs feed `PlumePhotometry`; USD connects its derived length fraction and light outputs to the WGSL material and scene-property ports on a fixed render envelope | ✅ (no script — physical length, light, and zero-thrust visibility are wired) |
