...
camera receives `CameraPathDriven` and
`lunco_camera_core::CameraPoseLock`; the path removes the mounted follower and
the camera runtimes honour the lock. This gives one
