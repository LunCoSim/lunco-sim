...
| [`48-assembly-editor.md`](48-assembly-editor.md) | The Assembly editor, including the generic Rhai model-authoring workflow for human and AI assembly creation |
...
| [`50-usd-driven-visuals.md`](50-usd-driven-visuals.md) | Beams, plumes, ribbons: geometry+look authored in USD, generic projection in Rust, policy in Rhai, bound by name (`info:id`). **`radius`/`height` bake at instantiation — live size is `xformOp:scale`**; a `lunco:*` property needs THREE files or it is inert |
...
| [`parametric-geometry-and-constraints.md`](parametric-geometry-and-constraints.md) | Typed f64 profile geometry, SysML-sourced constraint policy, standard USD output, and the staged toolkit gaps |
...
| [**`render-decoupling.md`**](render-decoupling.md) | **The material is the boundary.** Domain crates state appearance *intent* (`PbrLook`, `ShaderLook`, `SceneCamera`, `WorldLabel`); only `lunco-render-bevy` names `bevy_pbr`. It also records host-scoped Bevy state/input-focus features. This is why `--no-ui` links no wgpu/`bevy_render`/egui/winit — and why the `cargo tree` guard exists |
...
| [`terrain-substrate.md`](terrain-substrate.md) · [`terrain-layered-rendering.md`](terrain-layered-rendering.md) · [`terrain-editor-preview-handoff.md`](terrain-editor-preview-handoff.md) | The height oracle (one `HeightSource` from orbit to rover), the layered Data→Transfer→Blend rendering pipeline, and the Editor-preview terrain integration handoff |
| [`terrain-precompute-plan.md`](terrain-precompute-plan.md) | **Design.** Precomputed coarse tiles + monotone progressive refinement for the streamed surface. It records the historical measurements that established the current coarse-base choice |
| [`native-hook-providers.md`](native-hook-providers.md) | Twin-approved native implementations of reflected hook contracts, typed ABI/lifecycle boundaries, and the consumed terrain-kernel extension point |
...
| [`62-deterministic-runtime-and-async-boundaries.md`](62-deterministic-runtime-and-async-boundaries.md) | Cross-domain tick ordering, async preparation and revision admission, causal dependencies, numerical scope, and the remaining deterministic replay contract |
