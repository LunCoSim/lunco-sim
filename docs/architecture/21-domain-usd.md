...
3. **UsdSimPlugin** — Detects simulation schemas (`PhysxVehicleContextAPI`, `PhysxVehicleWheelAPI`, and the Omniverse differential/steering APIs `PhysxVehicleTankDifferentialAPI` (skid) / `PhysxVehicleAckermannSteeringAPI`) and creates `WheelRaycast`, `OutputPorts`, a data-driven `DriveMix` (allocated by a named kernel in `lunco-mobility`'s `ControlKernelRegistry` — `skid`/`linear`), `DifferentialCoupling`, sensors, etc. Also wires output topology + drive mix and cosim models/wires (see [`22-domain-cosim.md`](22-domain-cosim.md)).
