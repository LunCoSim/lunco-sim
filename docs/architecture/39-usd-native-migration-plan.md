...
- **P5.2 [in place] — Netlist-from-USD synthesizer + `SynthesizerRegistry`** (rhai policy + Rust
  ...
  and `register_hook_synthesizer` provides the authored-policy seam without replacing an existing typed
  domain owner. A hook receives Rust-owned composed network facts and returns the generated Modelica
  source, its explicit unit merge, and diagram placements; it never re-reads USD. The result is validated
