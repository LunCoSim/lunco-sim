...
- **Command Result (ACK)**: The **Feedback Loop** for the command fabric. Result-returning commands return `Result<Ack, String>` (`Ok` for success/ACK, `Err` for failure/NACK) on the original request, ensuring that the USER, scripts, or AI Agents have definitive confirmation of execution truth.
...
### Simulator as Mechanism, Embodiment as Agency
The simulation core (Level 1 & 2) is a "dumb" physical reactor. Intelligence and interaction (Level 3, 4, & 5) are delegated to pluggable agents. The **Embodiment** is the only entry point for human interaction, ensuring 100% decoupling between the physical plant and the pilot.
...
bindings. `Embodiment` itself is not a user or session and does not grant local
input authority. The `LocalEmbodiment` ownership marker identifies the one
embodiment eligible for this process's input and interactive camera; a
`RemoteEmbodiment` is another session's replicated embodiment. Session/control
...
### Controller
The "Pilot's Translator." It is a thin, logically "boring" bridge between the Embodiment's generic Actions and the Space System's specific Flight Software commands. It does NOT handle wheel mixing, steering, or system logic; those are delegated to the FSW.
...
### Intent Stream
The stream of high-level intents generated by the Embodiment (Level 5). Concretely
...
- **Silence is data**: a missing or stale sample remains visible to the owning controller. Any response to it is explicit controller policy, not an implicit stream fallback.
