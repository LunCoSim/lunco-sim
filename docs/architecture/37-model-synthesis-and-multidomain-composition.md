...
> `LunCoProgramAPI` prim, whose BOUNDARY ports (bus voltage, per-load power/current, pack SoC)
...
2. **Rule:** lock the two-level composition — acausal within domain (one DAE), causal across domains
   (scalar co-sim). Document the electrical layer as one `LunCoProgramAPI` prim + boundary ports.
