...
> `lunco-workbench-file-dialog` owns native and browser file-dialog backends; the
> shell and domain panels consume its typed request/result events.
>
...
lower-order pass clears and redraws that same texture every frame. Because
Bevy includes MSAA and HDR format in the texture key, the workbench's
change-driven `sync_egui_host_msaa` follows the active scene camera across
quality changes, camera replacement, resize, and Twin teardown. A private
...
- Embodiment and vessel input bindings are owned by `lunco-input-core` as the typed
  `InputBindingsSettings` section in `<OS config dir>/lunco/settings.json`. The
  application discovers the authored default document by its
...
```
  Apps
   ...
   │         │
   │         ▼
   ├── lunco-workbench-file-dialog  (optional dialog capability)
   │     - native rfd and wasm file-dialog/download backends
