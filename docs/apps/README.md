...
- **Command format:** `{"type":"ExecuteCommand","command":"<Name>","params":{...}}`. Meta queries use
  `{"type":"DiscoverSchema"}` / `{"type":"ListEntities"}`; `QueryEntity` is
  an `ExecuteCommand` call with `params: {"id":<n>}`.
