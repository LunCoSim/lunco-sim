# LunCoSim Operations & Deployment Guide

Build, deploy, and run LunCoSim — the native headless server and the
...
```
*Output → `dist/server/` (binary `luncosim` + `assets/`).*

### B. Web WASM client
```bash
./scripts/build_web.sh build luncosim --release
```
*Output → `dist/luncosim/`.*
...
```bash
./scripts/deploy_luncosim_server.sh deploy@host:~/luncosim-server                  # binary + assets
./scripts/deploy_luncosim_web.sh    deploy@host:/var/www/html/sandbox.lunco.space  # wasm (brotli/gzip pre-compressed)
```
...
```bash
cd ~/luncosim-server
./luncosim --host 5888 --api 4101 --cert /etc/letsencrypt/live/sandbox.lunco.space
```
...
./target/release/luncosim --connect sandbox.lunco.space

# Bare IP (LAN/dev): no cert needed — TLS validation is skipped.
./target/release/luncosim --connect 192.168.1.50
```
