...
| **`lunco-render`** | Render-free appearance intent and typed graphics settings; declares `render.quality_profile` and `render.default_quality_profile` while Rhai owns the profile values and fresh-settings choice. Names `Mesh3d`, never `MeshMaterial3d`. |
