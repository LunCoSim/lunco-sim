...
| **`lunco-render-bevy`** | The **only** crate that names `bevy_pbr`. Binds the intent (`PbrLook`/`ShaderLook`/`SceneCamera`/`WorldLabel`) to real materials & cameras; owns `ShaderMaterial`, capability-gated Bevy GPU camera culling, and conservative spotlight shadow relevance across active 3D views. Headless never adds it. |
