...
| **`lunco-modelica-ui-core`** | Render-independent Modelica UI contracts: shared command payloads (`OpenClass`, `FocusDocumentByName`) and stable plot identities. It has no Modelica compiler, workbench shell, panel, or renderer dependency; observers remain in the owning UI package. |
