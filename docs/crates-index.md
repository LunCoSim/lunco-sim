...
| **`lunco-scripting`** | Language/runtime-neutral scripting host and authored policy seams: backend lifecycle, document ownership, scenario execution, world bridge, and tool/timeline integration. **rhai** is an optional backend; Python is an optional one-shot-eval backend, Lua a reserved (unimplemented) backend id. Its bindings consume `lunco-scripting-bridge-core` rather than owning generic world access; reusable Rhai mechanics live in `lunco-scripting-rhai-core`. |
...
**`lunco-scripting`**
Language/runtime-neutral scripting host and authored policy seams. It owns document lifecycle, scenario execution, the world bridge, and backend integration. The **rhai** backend is optional and browser-capable; Python is an optional one-shot snippet backend, and Lua is a reserved backend id. Generic reflected world access is owned by `lunco-scripting-bridge-core` and bound here for each language. Reusable Rhai mechanics are isolated in `lunco-scripting-rhai-core`; authoring/query providers are installed by `lunco-scripting-rhai`.
...
Production Rhai authoring/query package. It owns catalog discovery, script diagnostics, and dataset queries, and is installed alongside `lunco-scripting` by Rhai hosts. Keeping these API/editor-facing providers separate limits rebuilds of the language-neutral scripting lifecycle when query surfaces change.
