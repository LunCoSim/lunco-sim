...
| **`lunco-scripting-bridge-core`** | Language-neutral reflected world bridge: native value construction, ECS reads/writes, command/query dispatch, hierarchy/spatial access, clocks, authority, and capability facts. It has no interpreter or UI policy; Rhai and Python adapters consume this shared mechanism. |
...
**`lunco-modelica-api`**
Production API capability for Modelica hosts. It registers the domain-owned query providers for bundled sources, source-library classes, compile/run status, experiment results, document source, model metadata, variables, and share links. `lunco-workspace-api` remains the owner of Workspace queries. Keeping this layer outside `lunco-modelica-core` means a headless compiler-only build does not pull the API query closure, while API-enabled UI/server roots install the required capabilities explicitly.
...
Language-neutral, interpreter-free world bridge for scripting backends. It owns native value construction, reflected ECS reads/writes, command/query dispatch, hierarchy and spatial queries, clocks, authority, capabilities, and the resolved scenario audience. Rhai and Python provide only their native value builders and language bindings. Keeping this mechanism separate lets hosts that need the bridge compile it independently from the larger scenario, policy, tool, and interpreter package.
