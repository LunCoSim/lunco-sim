...
| **`lunco-usd-sim-domain`** | Render-free USD domain projection: reads component-network facts, resolves Modelica member classes, and publishes generated Modelica sources. Its public `synthesis` module owns Rhai-backed synthesis policies and generated-plan contracts; runtime ECS projection remains in the parent module. Generic USD actuator lowering lives in `lunco-usd-actuation`; its optional API query providers live in `lunco-usd-sim-domain-api`. |
