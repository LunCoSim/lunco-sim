...
| **`lunco-scripting-rhai-runtime`** | Production Rhai application boundary: world bridge, `RhaiScenarioRuntime`, Rhai commands, policy activation, tool/timeline persistence, native hook providers, and `.rhai` asset dependency loading. It installs the language-neutral scripting host and owns the high-churn interpreter closure. |
...
Production Rhai application boundary. It owns the reflected world bridge and `RhaiScenarioRuntime`, Rhai commands, authored policy activation, tool/timeline persistence, native hook-provider lifecycle, and the `.rhai` asset/import graph. It depends on the language-neutral scripting host and the reusable `lunco-scripting-rhai-core` substrate, so changes to Rhai behavior no longer rebuild the generic scripting package.
