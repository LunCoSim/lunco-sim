...
| **`lunco-usd-sim-domain`** | Render-free USD domain projection: reads component-network facts, resolves Modelica member classes, invokes Rhai synthesizers, publishes generated Modelica sources, and owns generic USD actuator descriptions. It is independent of the vehicle/cosim orchestration crate. |
...
| **`lunco-scene-commands`** | The render-free scene/document **mutation layer**: runtime spawn, move, delete, camera, and USD connection edits. It is the shared command path for rhai, HTTP API, peer replay, and editor tools; catalog resources live in `lunco-scene-catalog`, while document-backed property and shader authoring live in `lunco-scene-authoring`. |
| **`lunco-scene-catalog`** | Production USD-backed catalog boundary: asynchronous asset enumeration, authored `doc`/`lunco:spawnable` metadata, shader/source listings, `SpawnCatalog`, and the generic runtime USD spawn constructor. It also owns catalog-only rescan commands and is installed by `SpawnCommandPlugin`. |
...
| **`lunco-scene-validation`** | Production asset, loaded-stage, and Twin pre-flight: `ValidateAsset`, `ValidateTwin`, live `RunLint`, USD lint-fact aggregation, and Twin namespace inspection. It owns composition/parse/lint integration while `lunco-scene-commands` owns scene mutation. |
...
resolves Modelica member classes, invokes authored Rhai synthesizers, publishes
generated Modelica sources, and lowers generic force/torque actuator
descriptions. The package has no dependency on the vehicle/cosim orchestration
crate, so Modelica-domain edits do not rebuild that orchestration source.
