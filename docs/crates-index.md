...
| **`lunco-terrain-surface`** | Local DEM ground terrain (surface scale): authoritative height queries and collider rings plus an opt-in `TerrainSurfaceVisualizationPlugin` for camera-driven visual LOD, derived maps, and overlays; one committed surface-change contract and independent product resolutions. |
...
| **`lunco-usd-queries`** | UI-free public USD query providers for document inspection, edit-session state, explicit assembly-target resolution, document synchronization, and read-only NURBS collision planning. `AvianMeshCollisionApproximations` exposes the runtime adapter's supported USD mesh modes as the authoring capability source. Public query behavior is covered by authored USD/Rhai scene tests. |
...
| **`lunco-sysml`** | SysML source asset/document lifecycle and journal integration; prepares manifest-selected Twin snapshots and open-document parser/resolver snapshots through shared async admission, fenced by Twin/root/operation or document-generation/origin. It consumes the pure AST projection and exposes typed state through the API adapter; it does not own Rhai policy. |
...
| **`lunco-hooks`** | Language-agnostic hook registry and shared typed `HookValue` ABI. Each `HookInvocation` carries positional arguments and the caller's typed runtime context; discrete calls use the explicitly unclassified entry point. The registry owns bounded wire encoding and exact registration admission/teardown. |
| **`lunco-hooks-rhai`** | Rhai backend for `lunco-hooks`: compiles a Rhai `source` + `entry` function, registers it under a hook id, and exposes the call's immutable clock/cycle facts as `runtime_context`. |
| **`lunco-hooks-plugin-api`** | Edition-2024 native-provider ABI v2: stable descriptor/capability structs, callback status protocol, and typed invocation wire helpers carrying arguments and runtime context. |
...
typed scene-lifecycle contracts, structured runtime faults/diagnostics, Bevy
`RuntimeCycleSet` ordering labels, and generic state utilities. Owner-neutral
cycle and invocation values are defined in `lunco-runtime-context`. Pure mutation envelopes are owned by
...
commands, and the stage terminal-outcome contract. Default Twin admission
parses the exact source revision and serializes the restored persistent
document snapshot through native `AsyncWorkAdmission`; it validates source text
and document generation before committing. Runtime sidecar restore remains
synchronous, and wasm worker transport is not installed here. It does not assemble the complete visual,
...
**`lunco-luncosim-runtime`**
Production application integration above the generic simulation substrate. It installs the Rhai runtime, projects USD-authored policies, applies active-Twin globe streaming overrides on Workspace changes, registers `SetRhaiPolicy`, consumes scripting/tool/timeline journal entries, and owns the public headless builders and launcher. Keeping this boundary above core prevents application policy changes from invalidating the generic simulation composition.
