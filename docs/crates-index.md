...
| **`lunco-scene-commands`** | The scene/document **command layer**: every runtime mutation — move, delete, set-property, shader edit — authored as journaled USD ops on the open document's runtime layer. One path for all four callers (rhai, HTTP API, peer over the wire, editor gizmo); an edit that bypasses it escapes save, journal, undo and replication. Catalog resources and discovery live in `lunco-scene-catalog` so catalog changes do not rebuild the command handlers. |
