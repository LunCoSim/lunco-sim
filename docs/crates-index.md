...
| **`lunco-camera-runtime`** | Generic interactive camera realization: camera-mode exclusivity, frame-handoff rebasing, and free-flight/surface pose writers over the camera-core contracts. Avatar and other authored operators supply their own source-specific frame and input policy. |
...
| **`lunco-sysml-ast`** | Pure SysML v2 parser/resolver projection: source files, qualified elements, typed literals, requirements, verification links, diagnostics, and neutral facts for the authored `lint.sysml` policy. It has no Bevy, filesystem, Twin, or scripting ownership. |
...
| **`lunco-scripting`** | Dynamic-language hosts and authored policy seams: **rhai** is the default (browser-capable) backend; Python is an optional one-shot-eval backend, Lua a reserved (unimplemented) backend id; logic providers cover scenarios and sequencing. Its bindings consume `lunco-scripting-bridge-core` rather than owning generic world access. |
...
| **`lunco-modelica-core`** | `lunica_worker`, `modelica_run`, `modelica_tester` | Headless Modelica compiler/worker runtime. It owns source access, parsing, compilation, and worker protocol, while native editor indexing/provisioning is composed by the Modelica UI. Numerical solver construction is supplied by `lunco-modelica-solver`; the worker protocol and live ECS component come from `lunco-modelica-runtime`. |
...
exclusivity and free-flight/surface pose writers live in
`lunco-camera-runtime`; optional egui presentation is supplied by
...
components, shared smoothing defaults, pure frame/zoom/movement math, authored
rig intent, camera-mode transition state, and pose-input accumulators. It does
not choose an avatar or a camera source.

**`lunco-camera-runtime`**
Generic interactive camera realization package. Owns the exclusive camera-mode
hooks, frame-handoff rebasing, and free-flight/surface pose writers over
`lunco-camera-core`. It is reusable by avatar, inspection, and other authored
operators; source-specific frame production and input policy remain with the
calling runtime.
...
`lunco-avatar-core`, and `lunco-avatar-policy`. It owns the avatar status
...
perspective presets (Build, Simulate), and concrete docking,
...
**`lunco-modelica-api`**
Production API capability for Modelica hosts. It registers the domain-owned query providers for bundled sources, source-library classes, compile/run status, experiment results, document source, model metadata, variables, and share links, together with the transport-free document edit commands. `lunco-workspace-api` remains the owner of Workspace queries. Keeping the edit/query surface outside `lunco-modelica-core` keeps the compiler host focused on document, compile, and simulation mechanisms; UI/server roots install the required API capability explicitly.

**`lunco-scripting`**
Dynamic-language hosts and authored policy seams. The default (and only fully-wired) backend is **rhai** — browser-capable and enabled by the default `rhai` feature; build with `--no-default-features` for a script-free host. Python is an optional backend used for one-shot snippet evaluation only; Lua is a reserved (not yet implemented) backend id. Rhai also funnels the `lunco-tools` registry into the engine via `lunco-tools-rhai`. Generic reflected world access is owned by `lunco-scripting-bridge-core` and bound here for each language.
