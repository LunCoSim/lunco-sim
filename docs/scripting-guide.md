...
- **Math:** `distance`, `arrived`, `vsub`/`vlen`/`vlen_squared`/`vnorm`/`vcross`, `clamp`.
...
- **Vector math:** `vsub`/`vadd`/`vlen`/`vlen_squared`/`vdot`/`vcross`/`vnorm`/`vscale`/`clamp`, `distance`, `arrived`. Use `vlen_squared(v)` when the squared magnitude is needed; it accepts native `Vec3` or a three-number array. Use native `Vec3`/`Quat` (`world_pos3`, `world_forward3`, `world_rotation_quat`) in hot loops; arrays are the explicit USD/telemetry interchange form and are lowered with `vec3_array`/`quat_array`.
...
- **Route programs** ([`route_follow.rhai`](../assets/scenarios/route_follow.rhai)): the scene owns an ordered USD route and a sibling `LunCoProgramAPI` program. The program resolves its `inputs:subject` relationship, reads route-point poses, initializes visited points from current sensor contacts, and advances an enabled route on generic sensor enter events. Route points are not stored on or discovered through a vessel-owned list.
- **Route presentation**: the reusable [`route_point.usda`](../assets/markers/route_point.usda) asset owns the standard translucent, unlit, shadowless visual/material and trigger geometry. Its unvisited point is green in standard `primvars:displayColor`/`primvars:displayOpacity`; the generic `route_follow` policy uses the `waypoint_editor` transient USD view tool to turn a visited point gray. A secondary `route.context` gesture opens the authored menu without selecting the waypoint or showing its gizmo; selection is an explicit menu action. A scenario consumes `route_point_reached` for mission policy without recreating distance checks or adding a second marker implementation.
- **Route restart and possession**: `route_follow` keeps visited USD point paths when autopilot is disabled and re-enabled, and selects the first unvisited point. `SensorOccupants` reads Avian's current touching moving bodies by stable sensor id, so the route marks an occupied starting point visited in `on_start`, before autopilot is enabled. Sensor enters after startup also mark visits while disabled; an active route advances on the matching enter event. Releasing manual possession triggers the generic control safe-stop; an enabled route claims the now-unowned rover, invalidates its cached target, and republishes the current waypoint so autopilot continues.
...
That namespace call does not declare an asset dependency. A scenario that needs
a source-defined tool before its first lifecycle hook should import the explicit
`lunco://` asset URI; see [Rhai asset dependencies](architecture/56-asset-resolution-and-cache.md#rhai-imports-use-the-shared-asset-identity).
...
- Discover: `ListToolLibraries`, `GetToolLibrary { name }`; each result includes
  the winning `standard`, `core`, `application`, or `twin:<id>` scope.
- The engine asset manifest supplies source inventory and the authored
  `scripting.source.classify` hook decides which sources are tools or prelude.
  Source admission tracks both the effective policy revision and whether that
  hook is installed. A Twin policy handoff that temporarily removes the hook
  retires its tools; restoring the hook re-admits them before script execution.
