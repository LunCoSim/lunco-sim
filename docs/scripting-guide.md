...
- **Route restart and possession**: `route_follow` keeps visited USD point paths when autopilot is disabled and re-enabled, and selects the first unvisited point. If the rover starts inside the route, the start transaction marks its consecutive occupied prefix visited before later progression is driven by Avian sensor events. Releasing manual possession triggers the generic control safe-stop; an enabled route claims the now-unowned rover, invalidates its cached target, and republishes the current waypoint so autopilot continues.
