...
./scripts/build_web.sh build lunica_web 2>&1 | grep wasm-opt
...
cargo build --release --target wasm32-unknown-unknown --bin lunica_web

# Generate JavaScript bindings
wasm-bindgen target/wasm32-unknown-unknown/release/lunica_web.wasm \
...
│       ├── lunica_web_bg.wasm   # WASM binary
│       ├── lunica_web.js        # JS glue
│       └── *.d.ts          # TypeScript declarations
├── src/
│   ├── bin/
│   │   ├── lunica.rs      # Desktop binary
│   │   └── lunica_web.rs  # Web entry point
...
```
dist/<binary>/
  lunica_web.js          # wasm-bindgen JS glue
  lunica_web_bg.wasm     # post-wasm-opt binary
  lunica_web.d.ts        # TypeScript declarations
...
| 404 on `lunica_web.js` | Stale `dist/` after layout change | Re-run `./scripts/build_web.sh build …` to regenerate |
