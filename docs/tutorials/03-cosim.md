...
declared communication points. That
...
#    (…see the asset for the full actuator-network edges.)
...
Touchdown is a Modelica state output consumed by the composed guidance boundary;
the attached lander supervisor turns the qualified landing transition into the
`lander_touchdown` mission event. The script contains no polling or physical
threshold math.
